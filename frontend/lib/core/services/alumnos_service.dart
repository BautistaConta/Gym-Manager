...
class AlumnosService {
  ...
  Future<List<AlumnoModel>> search(String nombre) async {
    ...
    final response = await http.get(
      Uri.parse(
        '${ApiConstants.baseUrl}/api/alumnos/search?nombre=${Uri.encodeQueryComponent(nombre)}',
      ),
      headers: {'Authorization': 'Bearer $token'},
    );
    if (response.statusCode != 200) throw Exception('Error buscando alumnos');
    return (jsonDecode(response.body) as List)
        .map((e) => AlumnoModel.fromJson(e))
        .toList();
  }
  ...
  Future<AlumnoModel?> getByDni(String dni) async {
    ...
    final response = await http.get(
      Uri.parse(
        '${ApiConstants.baseUrl}/api/alumnos/dni/${Uri.encodeComponent(dni)}',
      ),
      headers: {'Authorization': 'Bearer $token'},
    );
  ...
  Future<void> updateAlumno(
    ...
    final response = await http.put(
      Uri.parse('${ApiConstants.baseUrl}/api/alumnos/$id'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
    ...
    if (response.statusCode != 200)
      throw Exception('No se pudo actualizar el alumno: ${response.body}');
  }
