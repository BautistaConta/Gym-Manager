...
{
    private readonly CategoriaPagoService _service;
    public CategoriasPagoController(CategoriaPagoService service) => _service = service;
    ...
    [HttpGet("{id}")] public async Task<IActionResult> GetById(string id) => await _service.GetByIdAsync(id) is { } categoria ? Ok(categoria) : NotFound();
    [HttpPost] public async Task<IActionResult> Create(CrearCategoriaPagoRequest request) => await ExecuteAsync(() => _service.CreateAsync(request), value => CreatedAtAction(nameof(GetById), new { id = value.Id }, value));
    [HttpPut("{id}")] public async Task<IActionResult> Update(string id, ActualizarCategoriaPagoRequest request) => await ExecuteAsync(() => _service.UpdateAsync(id, request));
